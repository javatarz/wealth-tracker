...
Reads database URL from the application Settings so there is a single
source of truth for where the database lives.

ADR 0025: forward-only migrations — ``downgrade`` raises NotImplementedError.
"""
...
# Ensure the backend package is importable when Alembic runs from this directory.
